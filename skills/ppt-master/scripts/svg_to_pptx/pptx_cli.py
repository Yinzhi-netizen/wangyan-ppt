...
''',
    ...
    parser.add_argument('--cleanup', action='store_true', default=True,
                        help='Automatically clean up temporary directories '
                             '(svg_final/, old backups/) after successful export '
                             '(default: enabled).')
    parser.add_argument('--no-cleanup', action='store_true',
                        help='Disable automatic cleanup of temporary directories '
                             'after export. Equivalent to --cleanup=false.')
    ...
    # Automatic temp directory cleanup (enabled by default, disable with --no-cleanup)
    do_cleanup = args.cleanup and not args.no_cleanup
    if success and do_cleanup:
        # 1. Clean up svg_final/ when it was not needed for legacy export
        if not gen_legacy:
            ...
            if svg_final_dir.is_dir():
                try:
                    ...
                    if verbose:
                        print(f"  cleaned up svg_final/")
                except Exception as exc:
                    if verbose:
                        print(f"  [warn] svg_final cleanup skipped: {exc}")

        # 2. Clean up old backup directories, keep most recent 3
        ...
        if backup_root.is_dir():
            try:
                ...
                if verbose:
                    print(f"  [warn] backup cleanup skipped: {exc}")
